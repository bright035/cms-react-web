...
export const axiosGetCoursesByPage = async (paginator) => {
  return await axiosInstanceAuthen().get(
    `/courses?page=${paginator.page}&limit=${paginator.limit}`
  );
};

export const axiosGetStudentById = async (id) => {
  return await axiosInstanceAuthen().get(`/students/${id}`);
};

export const axiosGetCourseById = async (id) => {
  return await axiosInstanceAuthen().get(`/courses/detail?id=${id}`);
};
